 ...
 */
function wsf_infer_content_model( DomNode &$current_child, DomNode $sig_node, $classmap, $cid2cont_type, $cid2attachments ) {
    ...
    if($content_model == WSF_SIMPLE_CONTENT) 
    {
      ...
      if($text_node && $text_node->nodeType == XML_TEXT_NODE) 
      {
        ...
        $is_list = FALSE;
        if( $sig_node->attributes->getNamedItem(WSF_LIST) && $sig_node->attributes->getNamedItem(WSF_LIST)->value == "yes" ) 
        {
          $is_list = TRUE;
        }
        ...
        wsf_deserialize_type_info( $param_type, $parse_tree, WSF_SIMPLE_CONTENT_VALUE, $converted_value, $cid2cont_type, $cid2attachments );
		...
		if(array_key_exists($xsd_type, $xsd_php_mapping_table)) {
			...
			if($type == 'integer') {
				...
				$converted_value = ($data_value === "true");
					...
					$converted_value = $wsf_convert_utf8_decode ? utf8_decode($data_value):$data_value;
					$converted_value = preg_replace('/\%/','&#37 ', $converted_value);
				}
				else {
					$converted_value = $data_value;
				}

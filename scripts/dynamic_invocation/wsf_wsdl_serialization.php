...
function wsf_serialize_simple_types(DomNode $sig_param_node, $user_val,
    ...
    if($is_attribute) {
        ...
        if(!is_array($user_val) && !is_object($user_val) ||
            ...
            // {{{ kroknet - fix an issue on the custom soap_header
            if( '' == $attri_name )
              $attri_name = 'void';
            ...
            ws_log_write(__FILE__, __LINE__, WSF_LOG_ERROR, "attribute found -  ". $param_name ." which have given an array.");
        }
    }
    else {
        ...
        if($max_occurs > 1 || $max_occurs == "unbounded") {
            ...
            if(!($user_val && is_array($user_val)) ||
                ...
                ws_log_write(__FILE__, __LINE__, WSF_LOG_ERROR, "Array is given for ". $param_name ." which should be a non array.");
            }
        }
    }
}
...
function wsf_build_content_model(DomNode $sig_node, array $user_arguments,
    ...
    ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG, ""); // {{{ kroknet - print_r($user_arguments, TRUE));
                ...
                if($param_name == $user_key) {
                    if($user_val === NULL) {
                        if($sig_param_node->attributes->getNamedItem(WSF_MIN_OCCURS) &&
                            ...
                            ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG, "minOccurs=0 element $user_key is nil");
                            ...
                            ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG,  "nillable element $user_key is nil");
                        }
                        else {
                            // not allow to be NIL section
                            ws_log_write(__FILE__, __LINE__, WSF_LOG_ERROR, "non nillable and minOccurs!=0 element $user_key is nil");
                    ...
                    if (($sig_param_node->attributes->getNamedItem(WSF_WSDL_SIMPLE) &&
                        ...
                        ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG, "calling serialize simple types");
                        // default simple type
                        ...
                        ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG, "calling serialize complex types");
...
function wsf_wsdl_serialize_php_value($xsd_type, $data_value, 
    ...
    ws_log_write(__FILE__, __LINE__, WSF_LOG_DEBUG, "serializing ");// {{{ kroknet - $data_value);
    ...
    if($is_list && is_array($data_value)) {
        ...
        if($xsd_type == "anyType") {
            ...
            // {{{ kroknet - Avoid a issue in the custom soapheader
    ...
    return wsf_clean_serialized_value($serialized_value);
}

/**
 * kroknet fix
 * TODO: extend the characters to replace if any.
 */
function wsf_clean_serialized_value($serialized_value) {
    return str_replace('%', '&percent;', $serialized_value);
}

            ...
            $arg_count, $arguments, array $classmap = NULL, $mtom_on, &$attachment_map)
{
    ...
    {
      $payload_dom = new DOMDocument('1.0', 'UTF-8');
    ...
    return array(WSF_REQUEST_PAYLOAD => wsf_clean_payload($payload_str),
                WSF_INPUT_HEADERS => $input_headers);
}

/**
 * kroknet fix
 * TODO: extend the characters to replace if any (following the ones added in wsf_wsdl_serialization.php)
 */
function wsf_clean_payload($payload_str) {
     $payload_str = str_replace( '&amp;percent;', '%', $payload_str );
     return $payload_str;
}

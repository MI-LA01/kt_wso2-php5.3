...
function wsf_test_serialize_node($xml) {
    ...
    if ($xml->nodeType == XML_TEXT_NODE) { //for text node
        /**
         * {{{ Kroknet hack - This is to avoid some unrecognized characters 
         *     Like utf8 chars and % - The question is why !
         */
        //$nodeText = iconv("utf-8", "ascii//TRANSLIT", $xml->nodeValue);
        $nodeText = preg_replace('/\%/',"&#x25", $nodeText);

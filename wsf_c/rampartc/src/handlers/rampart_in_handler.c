    ...
    struct axis2_msg_ctx *msg_ctx)
{
    ...
    {
    	/**
    	 * {{{ Kroknet SOAP Header fixing issue
    	 *
    	 */
    	AXIS2_LOG_WARNING(env->log, AXIS2_LOG_SI, "[kroknet in rampart]SOAP header cannot be found - Also it can be simply a SOAP header missing from the partner (ie: generali ) Warning provided by kroknet!.");

    	soap_header = axiom_soap_header_create_with_parent(env, soap_envelope);

    	AXIS2_LOG_WARNING(env->log, AXIS2_LOG_SI, "[kroknet in rampart] - Defined a default soap header");
    }
    ...
    /** Fucking fix for a fucking missing header from generali ! kroknet give u the fuck ! **/
    if(soap_header) {
    	...
    	AXIS2_LOG_DEBUG(env->log, AXIS2_LOG_SI, "[kroknet in rampart] - a soap header exist");
    	AXIS2_LOG_DEBUG (env->log, AXIS2_LOG_SI, "[wsf_wsdl_kroknet] Response header is %s", response_header_buffer);
